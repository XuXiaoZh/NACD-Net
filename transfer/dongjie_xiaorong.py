...
CONFIG = {
    ...
    "output_dir":     r"D:/X/denoise/part1/v3/exp2_freeze",
...
# ============================================================
# 冻结策略定义（修正版）
# unfreeze 关键词与 model.named_modules() 中的名称对应
# 特殊值 "all" 表示解冻全部参数
# ============================================================
FREEZE_STRATEGIES = [
    {
    ...
    },
    {
        ...
        "unfreeze": ["denoiser"],                               # 只训练 denoiser，noise_encoder 冻结
    },
    {
        "name":    "freeze_noise_encoder",
        "display": "冻结NoiseEncoder（训练主干）",
        "unfreeze": ["denoiser"],                               # noise_encoder 冻结，denoiser 全量训练
    },
    {
        "name":    "full_unfreeze",
        "display": "全解冻（整体微调）",
        "unfreeze": ["all"],                                    # 全部参数可训练
    },
]
